...
/// FTDNA parser for Family Tree DNA raw data files
...
impl FTDNAParser {
    ...
    fn parse_data_line(&self, line: &str, data: &mut ParsedGeneticData) -> Result<()> {
        ...
        if parts.len() < 4 {
            return Err(anyhow!("Invalid FTDNA line: not enough columns"));
        }
        ...
        // For FTDNA files, we don't have explicit reference/alternate alleles
        ...
        let variant = Variant {
            ...
            position,
            rsid: Some(rsid),
            reference_allele: "".to_string(), // Not available in FTDNA format
            alternate_alleles: vec![],        // Not available in FTDNA format
            genotype,
            quality: None,
            filter: None,
...
impl super::GeneticDataParser for FTDNAParser {
